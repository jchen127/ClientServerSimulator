...
export class corelogicService{
  ...
  /***
   * Generates juvenile database entry if the total entry is less than 100
   * @param num
   * @private
   */
  public generateServerJuveniles(num: number): boolean{

    if(this.userService.getSecurityToken().length === 0){
    ...
    if(num > 0){
      ...
      //checking total number of juveniles
      //if 100 or more do nothing. otherwise insert


      //keep generating the juveniles until it hits 100 juveniles. stop at 100 juveniles

      let genderizer = () => { let num1:number = this.drawRandomNumber(2); if(num1<1){return "male";}else{return "female";} }

      for(let i=0; i<num; i++){
        this.generateOneJuvenile(
          new JuvenileModel(this.names[this.drawRandomNumber(this.names.length)],
            genderizer(),
            "Nobody cares about this useless juvenile description",
            this.drawRandomNumber(18),
            this.drawRandomNumber(100),
            this.drawRandomNumber(5),
            this.drawRandomNumber(4),
            this.drawRandomNumber(2),
            `/assets/images/juvenile/d${this.drawRandomNumber(15)}.svg`

            )
        );
      }
/*
      this.generateOneJuvenile(new JuvenileModel(
        'Mandy', 'Female',
        'Mandy was a kind and nice girl. Her family went bankrupt and were jailed. She was left in the care of her abusive uncle where she slowly succumbed into juvenile delinquency. ' +
        ...
        `/assets/images/juvenile/d11.svg`
      ))
*/
  ...
  private generateOneJuvenile(singleJuvenile: JuvenileModel){

    if(singleJuvenile === null || singleJuvenile === undefined)
      return;

    //let accessParam = new HttpParams();
   // accessParam.append("access_token", this.userService.getSecurityToken());
    let urlString: string = "http://localhost:3000/api/juveniles?access_token="+this.userService.getSecurityToken();
    this.http.post(urlString,
      {
      name: singleJuvenile.name,
        gender: singleJuvenile.gender,
        age: singleJuvenile.age,
        delinquencyScore: singleJuvenile.delinquencyScore,
        hitpoint: singleJuvenile.HitPoint,
        emotionalScore: singleJuvenile.emotionalScore,
        currentStatus: singleJuvenile.currentStatus,
        imageSrc: singleJuvenile.imageSrc
      }
  ...
  /***
   * return true if less than 100 juvenile. false otherwise.
   *
   *
   * @private
   */
  public JuvenileNumGreaterThan100(): boolean{
    let countUrl: string = "http://localhost:3000/api/juveniles/count";
    this.http.get<{'count': number}>(countUrl).subscribe(x => {
      if(x.count >= 100){

        return true;
      }else{
        console.log("Generating Juvenile Pool...");
        this.generateServerJuveniles(100);
        return false;
      }
      });
  ...
  constructor(public logServe: LoggingService, private http: HttpClient, private userService: UserService) {
    ...
    * */
    ...
    this.juveniles.push(
      ...
      new JuvenileModel(
        ...
        'Now she hates everyone around. Maybe someday, she will get better. Someday, but that someday is not today.', 15, 80, 10, EmotionScore.Uncooperative, PersonStatus.Healthy,
        `/assets/images/juvenile/d13.svg`
      )
